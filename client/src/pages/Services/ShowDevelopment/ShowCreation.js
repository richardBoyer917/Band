    ...
    <section>
      ...
      <div className="flexWrapAround" style={{ paddingTop: '35px', gap: '10px' }}>
        {creationInfo.map((item, index) => (
          <div key={index} className="creationItem" style={{ width: index === 0 && '320px', background: item.bgColor, marginLeft: index === 0 ? 0 : index === 1 && '-45px' }}>
            ...
            {item.contentText ?
              ...
              <div>
                <DarkIconButton icon={item.buttonIcon} title={item.buttonTitle} />
                <p className="docuSizeText" style={{ marginTop: 'clamp(16px , 2vw, 25px' }}>{item.sizeText}</p>
              </div>}
          </div>
        ))}
      </div>
