...
const RentalCost = () => {
  ...
  const RentalDocument = ({ documentFile }) => (
    <div className="flexWrapBetween">
      {rentalDocumentInfo.map((item, index) => (
        <RentalCostDocCard docFile={documentFile && documentFile[index] && documentFile[index]} key={index} item={item} />
      ))}
    </div>
  )
  ...
  const content = (
    <section>
      <p className="sectionTitle" style={{ color: `var(--primaryBgColor)`, width: '90%' }}>Стоимость аренды базы</p>
      <div className="rentalBody">
        ...
        <div className="rentalRight">
          <div className="rentalRightTextSquare">
            <p className="cardTitle" style={{ marginBottom: 'clamp(20px, 3vw, 35px)' }}>За отдельную плату может быть установлен любой технический сетап и предоставлены специалисты по свету, звуку и видео</p>
